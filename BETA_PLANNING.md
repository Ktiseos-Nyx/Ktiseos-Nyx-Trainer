...
- **Hinge FALSIFIED (2026-06-22):** the "pure consumer / no patch-set" claim was wrong — we DO carry a live patch-set (see 2026-06-22 decision above). A clean tracking fork was never actually on the table, which is a primary reason the submodule lost its edge. Plan instead: **PR our fixes upstream to 67372a** (the cold-file optimizer fixes + the SDXL-NaN root-cause are clean PRs); as they merge, the patch-set shrinks and the vendored tree stays close to upstream with less re-apply cost.
