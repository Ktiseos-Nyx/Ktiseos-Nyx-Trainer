...
| Qwen-Image VAE reflection padding - Anima (VAE-EQ-2) | Advanced Feature | Small (needs research) | ⏳ Not started — **feasibility is a CODE question**: the Anzhc & Bluvoll EQ-VAE patch (obtained via Jelosus2's fork) is SDXL-specific; whether it ports to Anima's VAE path (AuraFlow/Qwen autoencoder) must be checked in the Anima VAE code first before scoping. |
