...
// The GitHub code-search fallback for unknown nodes (comfyui-github-search +
// /api/comfyui-nodes) is deferred to a later dataset-tools slice — matches the
// STUB markers in components/dataset-tools/metadata-panel.tsx. Classification
// here uses only the extension-node-map registry + the built-in node set.
