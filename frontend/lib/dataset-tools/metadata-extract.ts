/**
 * AI-generation metadata extraction — PNG/JPEG chunk + EXIF + XMP parsing,
 * ComfyUI workflow graph tracing, and A1111/ComfyUI/NovelAI/Midjourney/etc.
 * format detection.
 *
 * Split out of the `/api/dataset-tools/metadata` route handler: Next.js's
 * App Router route contract only allows route.ts files to export HTTP verb
 * handlers (GET/POST/...) plus a small set of config values — any other
 * named export (like `extractMetadataFromBuffer`, needed by both the
 * `metadata` and `metadata-from-file` routes) fails Next's generated route
 * type-check. This module has no route contract, so the full extraction
 * pipeline — and its export shared across both routes — lives here instead.
 */
