  ...
  return (
    <div className="space-y-6">
      <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm text-blue-700 dark:text-blue-400">
        Bake one or more LoRAs directly into a base checkpoint, producing a standalone full model — like SuperMerger&apos;s LoRA merge. SD1.5 and SDXL only.
      </div>

      <SectionCard title="Architecture">
        <div className="grid grid-cols-2 gap-3">
          {(['sd', 'sdxl'] as const).map(t => (
            <Button key={t} variant={modelType === t ? 'default' : 'outline'} onClick={() => setModelType(t)}>
              {t === 'sd' ? 'SD 1.5' : 'SDXL'}
            </Button>
          ))}
        </div>
